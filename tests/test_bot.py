...
class TestPaymentVerificationBot:
    """Test cases for PaymentVerificationBot class."""

    @pytest.mark.asyncio
    async def test_check_message_content_with_payment_terms(self, mock_config, mock_templates, test_logger):
        ...
        bot = PaymentVerificationBot(test_logger, mock_config, mock_templates)
        for content, expected in test_cases:
            assert bot._check_message_content(content) == expected


    @pytest.mark.asyncio
    async def test_check_message_content_with_disclosure(self, mock_config, mock_templates, test_logger):
    ...
    @pytest.mark.asyncio
    async def test_check_message_content_invalid(self, mock_config, mock_templates, test_logger):
    ...
    @pytest.mark.asyncio
    async def test_handle_violation(self, mock_config, mock_templates, mock_message, test_logger):
        ...
        assert any("Blocked message from" in str(call) for call in test_logger.info.call_args_list)

    @pytest.mark.asyncio
    async def test_handle_violation_forbidden(self, mock_config, mock_templates, mock_message, test_logger):
        ...
        assert any("Failed to delete message" in str(call) for call in test_logger.warning.call_args_list)

    @pytest.mark.asyncio
    async def test_on_message_handler(self, mock_config, mock_templates, mock_message, test_logger):
